...
// Скільки в партії було спочатку. Потрібне звіту «куплено за період»: рахувати
// його від залишку означало б, що закритий місяць тихо переписується щоразу,
// коли з тієї партії щось продають. Для наявних рядків беремо поточний
// залишок — точнішого джерела заднім числом немає.
if (addCol("inventory_lots","qty_total","REAL DEFAULT 0")) {
  db.exec("UPDATE inventory_lots SET qty_total=qty_left WHERE COALESCE(qty_total,0)=0");
}
